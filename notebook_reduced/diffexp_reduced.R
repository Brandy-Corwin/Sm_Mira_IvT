...
# For star
star_counts_df <- read_tsv("counting_reduced/counts_reduced/star_counts.tsv", comment = "#") |>
...
write.csv(assay(star_vsd), "count_matrices_reduced/star_vsd.csv")
...
# For hisat
hisat_counts_df <- read_tsv("counting_reduced/counts_reduced/hisat_counts.tsv", comment = "#") |>
             mutate(across(where(is.numeric), as.integer))
...
hisat_counts_summary <- hisat_counts_df |>
    ...
    rename_with(~str_remove(., "counting_reduced/dedup_reduced/hisat.bam:"), everything()) |>
    rowwise() |>
    mutate(total_counts = sum(c_across(where(is.numeric)), na.rm = T)) |>
    filter(total_counts >= 10)
...
hisat_dist_plot <- hisat_dists_df |>
    ...
    coord_equal() +
    NULL
ggsave("plots_reduced/hisat_dist_plot_before_diffexp.png")
...
write.csv(assay(hisat_vsd), "count_matrices_reduced/hisat_vsd.csv")

hisat_pca_after_deseq2 <- plotPCA(hisat_vsd, intgroup = 'tissue')
ggsave("plots_reduced/hisat_pca_after_deseq2.png")
