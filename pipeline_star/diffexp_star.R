...
# For star
star_counts_df <- read_tsv("star_counting/counts/star_counts.tsv", comment = "#") |>
             mutate(across(where(is.numeric), as.integer))
write.csv(star_counts_df, "star_count_matrices/star_counts_df.csv")

star_counts_summary <- star_counts_df |>
    select(Geneid, contains('star.bam')) |>
    rename_with(~str_remove(., "star_counting/dedup/star.bam:"), everything()) |>
    rowwise() |>
    mutate(total_counts = sum(c_across(where(is.numeric)), na.rm = T)) |>
    filter(total_counts >= 10)
write.csv(star_counts_summary, "star_count_matrices/star_counts_summary.csv")
...
star_sample_summary <- star_counts_df |>
    ...
    tally() |>
    filter(n <= 3)
write.csv(star_sample_summary, "star_count_matrices/star_sample_summary.csv")
...
write.csv(star_counts_m, "star_count_matrices/star_counts_m.csv")
...
star_dist_plot <- star_dists_df |>
    ...
    coord_equal() +
    NULL
ggsave("star_plots/star_dist_plot_before_diffexp.png")
...
star_pca_plot_before_deseq2 <- star_pca_fit |>
    ...
    geom_point(size = 5)
ggsave("star_plots/star_pca_before_deseq2.png")
...
star_volcano_plot <- star_volcano_data |> 
    ...
    geom_vline(xintercept = -2)
ggsave("star_plots/star_volcano_plot.png")

star_vsd <- varianceStabilizingTransformation(star_dds)
write.csv(assay(star_vsd), "star_count_matrices/star_vsd.csv")

star_pca_after_deseq2 <- plotPCA(star_vsd, intgroup = 'tissue')
ggsave("star_plots/star_pca_after_deseq2.png")
